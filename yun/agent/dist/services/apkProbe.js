...
exports.runReadinessProbe = runReadinessProbe;
exports.tryAdoptReadyInstance = tryAdoptReadyInstance;
exports.applyCarProfile = applyCarProfile;
 ...
 * 执行完整就绪探针流程（供外部调用）
 */
async function runReadinessProbe(instance) {
    try {
        ...
        await (0, instanceManager_1.execShell)(`adb -s ${instance.adbTarget} forward --remove tcp:${instance.probePort} || true`);
        await (0, instanceManager_1.execShell)(`adb -s ${instance.adbTarget} forward tcp:${instance.probePort} tcp:8080`);
 ...
 */
async function tryAdoptReadyInstance(instance) {
    try {
 ...
 */
async function applyCarProfile(instance, carBrand, profileIndex, profileName) {
    try {
...
async function getCatalogBrands(instance) {
...
async function getCatalogProfiles(instance, brand) {
    return withProbeSocket(instance, async (ws) => {
 ...
 */
async function runBusinessProbe(instance, brand, profileIndex, profileName) {
    const wsUrl = `ws://localhost:${instance.probePort}/ws`;
    await runBusinessProbeOnUrl(instance.id, wsUrl, brand, profileIndex, profileName);
}
async function runBusinessProbeOnUrl(instanceId, wsUrl, brand, profileIndex, profileName) {
    ...
    try {
        ...
        if (profileIndex < 0 || profileIndex >= profiles.length) {
            throw new Error(`品牌 ${brand} 的配置下标越界: index=${profileIndex}, count=${profiles.length}, name=${profileName || '-'}`);
        }
...
async function withProbeSocket(instance, runner) {
    const wsUrl = `ws://localhost:${instance.probePort}/ws`;
    const ws = await connectWebSocket(wsUrl);
    try {
...
async function startApkActivity(instance) {
    await (0, instanceManager_1.execShell)(`adb -s ${instance.adbTarget} shell am start -n ` +
        `${instance.packageName}/${instance.activityName}`);
}
async function stopApkActivity(instance) {
    await (0, instanceManager_1.execShell)(`adb -s ${instance.adbTarget} shell am force-stop ${instance.packageName}`);
}
async function ensureAdbTransport(instance) {
    await (0, instanceManager_1.execShell)(`adb connect ${instance.adbTarget}`);
    await sleep(1000);
    await (0, instanceManager_1.execShell)(`adb -s ${instance.adbTarget} wait-for-device`);
    await sleep(500);
}
/**
 * 解析 A端响应的 Data 字段
 ...
 */
async function waitForApkReady(wsUrl, timeoutMs, brand) {
 ...
 * 建立 WebSocket 连接
 */
function connectWebSocket(url, timeoutMs = 5000) {
 ...
 */
function sendRequest(ws, action, data) {
    return new Promise((resolve, reject) => {
