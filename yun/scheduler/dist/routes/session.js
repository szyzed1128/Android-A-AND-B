...
router.post('/init', async (req, res) => {
    try {
        ...
        const session = await (0, tokenService_1.initSession)(deviceId);
        const resp = {
            success: true,
...
router.put('/:id/device', async (req, res) => {
    try {
        const session = await (0, tokenService_1.validateSession)(req.params.id);
        if (!session)
...
router.post('/:id/reserve', async (req, res) => {
    try {
        ...
        const { wsUrl, instanceId } = await (0, scheduler_1.reserveInstance)(session);
        const resp = {
            success: true,
...
router.post('/:id/cancel-reserve', async (req, res) => {
    try {
        ...
        if (session.instanceId && session.status === 'reserved') {
            await redis.updateInstanceStatus(session.instanceId, 'idle', {}, ['sessionId', 'reservedForDeviceId', 'reservedUntil']);
            await (0, scheduler_1.syncInstanceIdlePool)(session.instanceId);
        }
...
router.post('/:id/release', async (req, res) => {
    try {
        ...
        if (session.instanceId) {
            await (0, scheduler_1.releaseInstance)(session.instanceId, '用户放弃或连接失败');
        }
        const redis = (0, redis_1.getRedis)();
        await redis.updateSession(session.sessionId, { status: 'init' }, ['instanceId']);
        return res.json({ success: true });
...
router.post('/:id/heartbeat', async (req, res) => {
    try {
        const ok = await (0, tokenService_1.updateHeartbeat)(req.params.id);
        if (!ok)
            return res.status(404).json({ success: false, error: '会话不存在' });
        return res.json({ success: true });
    }
...
router.post('/:id/disconnect', async (req, res) => {
    try {
        ...
        if (!session)
            return res.status(404).json({ success: false, error: '会话不存在' });
        await (0, scheduler_1.disconnectSession)(session);
        return res.json({ success: true });
    }
    catch (err) {
        return res.status(500).json({ success: false, error: err.message });
