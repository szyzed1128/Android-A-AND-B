...
const catalogSnapshot_1 = require("../services/catalogSnapshot");
...
async function getIdleCatalogCandidates() {
...
async function fetchBrands(candidates) {
...
async function fetchProfilesForBrand(brand, candidates) {
...
async function main() {
    ...
    const snapshot = (0, catalogSnapshot_1.buildCatalogSnapshot)(brands, profilesByBrand, {
        instanceId: source.id,
        serverIp: source.serverIp,
        agentPort: source.agentPort,
    });
    const snapshotPath = await (0, catalogSnapshot_1.writeCatalogSnapshot)(snapshot);
    const redis = (0, redis_1.getRedis)();
