using System;
using System.Globalization;
using System.Linq;
using System.Runtime.CompilerServices;
using CarScannerXamarinForms.OBD2.PIDS;
using CarScannerXamarinForms.ViewModels;
using Xamarin.Forms;

namespace CarScannerXamarinForms
{
	// Token: 0x02000080 RID: 128
	public class PID_IDToPIDNameConverter : IValueConverter
	{
		// Token: 0x060002A1 RID: 673 RVA: 0x000188D4 File Offset: 0x00016AD4
		public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
		{
			if (value == null || !(value is int))
			{
				return PID.Empty.Name;
			}
			int id = (int)value;
			PID pid;
			if (id < 1001)
			{
				pid = App.OBDReader.CurrentCarData.LiveDataPIDs.FirstOrDefault((PID x) => x.Id == id);
			}
			else
			{
				pid = CustomPIDViewModel.CurrentCustom.PidCollection.FirstOrDefault((CustomPID x) => x.Id == id);
				if (pid == null)
				{
					pid = CustomPIDViewModel.CurrentProfile.PidCollection.FirstOrDefault((CustomPID x) => x.Id == id);
				}
			}
			if (pid == null)
			{
				return PID.Empty.Name;
			}
			return pid.Name;
		}

		// Token: 0x060002A2 RID: 674 RVA: 0x00018897 File Offset: 0x00016A97
		public object ConvertBack(object value, Type targetType, object parameter, CultureInfo culture)
		{
			if (value != null && value is PID)
			{
				return (value as PID).Id;
			}
			return PID.Empty.Id;
		}

		// Token: 0x060002A3 RID: 675 RVA: 0x00002050 File Offset: 0x00000250
		public PID_IDToPIDNameConverter()
		{
		}

		// Token: 0x02000081 RID: 129
		[CompilerGenerated]
		private sealed class <>c__DisplayClass0_0
		{
			// Token: 0x060002A4 RID: 676 RVA: 0x00002050 File Offset: 0x00000250
			public <>c__DisplayClass0_0()
			{
			}

			// Token: 0x060002A5 RID: 677 RVA: 0x0001898E File Offset: 0x00016B8E
			internal bool <Convert>b__0(PID x)
			{
				return x.Id == this.id;
			}

			// Token: 0x060002A6 RID: 678 RVA: 0x0001898E File Offset: 0x00016B8E
			internal bool <Convert>b__1(CustomPID x)
			{
				return x.Id == this.id;
			}

			// Token: 0x060002A7 RID: 679 RVA: 0x0001898E File Offset: 0x00016B8E
			internal bool <Convert>b__2(CustomPID x)
			{
				return x.Id == this.id;
			}

			// Token: 0x040001AD RID: 429
			public int id;
		}
	}
}
